...
class testbench():
    ...
    def ringtone_test(self, nfft: int = 256, noverlap: int = -1, visualize_output: bool = False):
        ...
        for i in range(result.shape[1]):
            result[:, i] *= scipy.signal.windows.hann(nfft, sym=True)
        #     result[:, i] = np.fft.fftshift(self.get_psd(result[:, i], nfft, sample_rate))
        ...
        f, t, stft_results = scipy.signal.stft(ts, fs=sample_rate, window="hann", nperseg=256, noverlap=None if noverlap == -1 else noverlap, nfft=nfft, detrend=False, return_onesided=False, boundary=None, padded=False, axis=-1, scaling='psd')
        # print(f)
        # print('\n')
        # print(t)
        # print('\n')
        ...
        windowed_fft = 10.0 * np.log10( np.abs(fft_result)**2 / (nfft*sample_rate) )
        print("isclose: {}".format(np.isclose(result[:, 0], fft_result)))
        print(result[:10, 0])
        print('\n')
        print(fft_result[:10])
        print('\n')
